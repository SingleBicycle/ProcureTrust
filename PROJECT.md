...
## 3. Orchestrator: Stage Machine (A/B/C/D) + Ready Gate
Stages:
...
- Backend: FastAPI (Python) + LangGraph Orchestrator
- DB: Postgres (jsonb) + optional pgvector
- Cache/Queue: Redis
- Assets: S3-compatible storage
- Frontend: Next.js (React)
- Observability: structured logs + traces + metrics dashboard endpoints

## 9. Repo Structure (monorepo)
- apps/api        # FastAPI + LangGraph + services
- apps/web        # Next.js UI
- packages/schemas# shared schema + codegen
